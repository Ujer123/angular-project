...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    // Periodically refresh data from the server
  ...
  duplicateTask(product: any) {
    ...
    const duplicatedProduct = { ...product, _id: this.generateUniqueId() };
  
    // Send a POST request to store the duplicated product in the backend
    this.http
      .post('https://task-backend-tfp7.onrender.com/products', duplicatedProduct)
      .subscribe(
        (response: any) => {
          console.log('Duplicated product stored in the backend:', response);
  
          // Add the duplicated product to the frontend list
          this.filteredProducts.push(response);
          this.products.push(response); // Ensure the main list is also updated
        },
        (error) => {
  ...
  deleteTask(product: any) {
    if (product._id) {
      console.log('Attempting to delete task with ID:', product._id);
        ...
        .subscribe(
          () => {
            ...
            this.filteredProducts = this.filteredProducts.filter(
              (p) => p._id !== product._id // Use _id instead of id
            );
